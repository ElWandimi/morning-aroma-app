...
export function AdminProducts() {
  ...
  return (
    <div>
      ...
      <button className="btn-outline small" style={{ marginBottom: 14 }} onClick={() => setShowAddForm((v) => !v)}>
        {showAddForm ? "Cancel" : "+ Add new product"}
      </button>

      {showAddForm && (
        <form className="admin-add-form" onSubmit={submitNewProduct}>
          ...
          <label className="filter-label" htmlFor="np-photo" style={{ marginTop: 14 }}>Product photo (optional)</label>
          <div className="admin-photo-upload-row">
            {formPhotoUrl && <img src={formPhotoUrl} alt="" className="admin-photo-preview" />}
            <div>
              <input id="np-photo" type="file" accept="image/*" onChange={handleNewProductPhoto} />
              <p className="hint" style={{ marginTop: 4 }}>Without one, the product shows its origin's landscape photo instead.</p>
              {photoUploadError && <p className="form-error">{photoUploadError}</p>}
            </div>
          </div>

          {formError && <p className="form-error">{formError}</p>}
          <p className="hint" style={{ marginTop: 8 }}>Appears immediately on Shop, its own product page, and can be added to cart — but not on Home's featured tiers or Moments/Brew Guides suggestions, which stay curated.</p>
          <button className="btn-primary" type="submit" style={{ marginTop: 10 }}>Add product</button>
        </form>
      )}
